...
describe('jwt', () => {
  ...
  it('rejects tampering, wrong secret, other algorithms and garbage', () => {
    const t = signJwt({ a: 1 }, secret);
    const [h, , s] = t.split('.');
    expect(verifyJwtSignature(`${h}.${b64({ a: 2 })}.${s}`, secret)).toEqual({
      error: 'BAD_SIGNATURE',
    });
    expect(verifyJwtSignature(t, 'x'.repeat(32))).toEqual({ error: 'BAD_SIGNATURE' });
    expect(verifyJwtSignature(`${b64({ alg: 'none' })}.${b64({ a: 1 })}.`, secret)).toEqual({
      error: 'BAD_ALG',
    });
  ...
  it('mints SSO tokens with iss/aud/exp=60s', () => {
    const r = verifyJwtSignature(mintSsoToken(secret, 'figura', 'Ops', 100), secret) as {
