...
export const RUN_STATUSES = ['draft', 'queued', 'preparing', 'running', 'reporting', 'cleaning', 'done', 'failed', 'refused', 'cancelled'] as const;
...
export const runConfigSchema = z
  .object({
    ...
    seed: z.number().int().min(0).max(2 ** 31).nullable().default(null),
    ...
    fakeScenario: z.union([z.string(), z.record(z.string(), z.unknown())]).nullable().default(null),
    allowCheckout: z.boolean().default(false),
    priceScenarios: z
      .array(z.object({ name: z.string().min(1), prices: z.record(z.string(), z.number().min(0)) }).strict())
      .default([]),
