...
describe('fake admin console', () => {
  it('embeds the app with a fresh SSO token and offers fallbacks', async () => {
    ...
    expect(page.body).toMatch(/<iframe id="app" title="Synthetic users simulator" src="http:\/\/localhost:4000\/#sso=[\w-]+\.[\w-]+\.[\w-]+"/);
    ...
    const verified = verifyJwtSignature(tok.json.token as string, SSO) as { payload: Record<string, unknown> };
  ...
  it('is local only', async () => {
    ...
    expect((await allowed.app.inject({ method: 'GET', url: '/console/token', remoteAddress: '172.18.0.5' })).statusCode).toBe(200);
  });
});
