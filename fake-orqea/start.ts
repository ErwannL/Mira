...
function required(env: Env, key: string, minLength = 1): string {
  ...
  if (v.length < minLength)
    throw new Error(`${key} must be set (at least ${minLength} characters)`);
  return v;
}

export function fakeConfigFromEnv(env: Env): {
  config: FakeConfig;
  scenario: Scenario;
  port: number;
  host: string;
  ...
  if (stripe !== 'test' && stripe !== 'live' && stripe !== 'off')
    throw new Error('FAKE_STRIPE_MODE must be test, live or off');
  const scenarioInput = env.FAKE_SCENARIO_FILE
    ? JSON.parse(readFileSync(env.FAKE_SCENARIO_FILE, 'utf8'))
    : (env.FAKE_SCENARIO ?? 'baseline');
  return {
    config: {
      ...
      adminAllowed: (env.FAKE_ADMIN_ALLOWED ?? '')
        .split(',')
        .map((s) => s.trim())
        .filter(Boolean),
