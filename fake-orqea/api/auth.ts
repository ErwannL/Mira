...
import { checkPassword, hashPassword, type Store } from '../store.js';
...
function authApi1(
  app: FastifyInstance,
  deps: Deps,
  h: { store: Store; brakes: Map<string, number[]> },
): void {
  const { store } = h;
  app.post('/api/auth/register', async (req, reply) => {
    ...
    if (!STRONG.test(String(b.password ?? '')))
      return fail(reply, ctx, 400, 'WEAK_PASSWORD', 'err_password');
    const extras = EXTRA_FIELDS.slice(0, ctx.scenario.extraSignupFields);
    if (extras.some((k) => !String(b[k] ?? '').trim()))
      return fail(reply, ctx, 400, 'MISSING_FIELD', 'err_required');
    if (b.acceptedTerms !== true) return fail(reply, ctx, 400, 'TERMS_REQUIRED', 'err_terms');
    if (captchaRequired(reply, ctx) && b.captcha !== true)
      return fail(reply, ctx, 400, 'CAPTCHA', 'err_captcha');
...
function authApi2(
  ...
  app.get('/api/auth/verify-email', async (req, reply) => {
    ...
    if (!user) {
      ...
      return html
        ? reply
            .type('text/html')
            .send(layout(ctx.view, t('invalidLink'), `<h1>${esc(t('invalidLink'))}</h1>`))
        : { error: 'INVALID_TOKEN' };
    }
    ...
    return reply
      ...
      .send(
        layout(
          view,
          t('emailVerified'),
          `<h1>${esc(t('emailVerified'))}</h1><p>${esc(t('emailVerifiedText'))}</p><a href="/login">${esc(t('logIn'))}</a>`,
        ),
      );
  });
  ...
  app.post('/api/auth/login', async (req, reply) => {
    ...
    if (!ctx.runId && recent.length >= 5)
      return reply
        .code(429)
        .send({ error: 'TOO_MANY_ATTEMPTS', message: 'Too many attempts, wait a minute.' });
    const user = store.userByEmail(
      String(b.email ?? '')
        .trim()
        .toLowerCase(),
    );
