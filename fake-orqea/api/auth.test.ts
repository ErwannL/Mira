...
describe('auth api', () => {
  it('registers (201), refuses duplicates (409) and explains 400s', async () => {
    ...
    expect(await bad({ ...good, email: 'nope' })).toMatchObject({ error: 'INVALID_EMAIL', message: expect.stringContaining('name@example.com') });
  ...
  it('extra required fields, unclear errors and captcha scenarios', async () => {
    ...
    expect((await f.call('POST', '/api/auth/register', { ...filled, captcha: true })).status).toBe(201);
    const run = { 'x-synthetic-run': signRunHeader('r1', SECRET, NOW) };
    const s = await f.call('POST', '/api/auth/register', { ...filled, email: 'synth+r1-student@synthetic.invalid' }, run);
  ...
  it('verifies email through the link, as HTML or JSON', async () => {
    ...
    const html = await f.call('GET', `/api/auth/verify-email?token=${u.verifyToken}`, undefined, { accept: 'text/html' });
    expect(html.body).toContain('Adresse vérifiée');
    expect(u.verified).toBe(true);
    expect((await f.call('GET', `/api/auth/verify-email?token=${u.verifyToken}`)).json).toEqual({ verified: true });
    const badHtml = await f.call('GET', '/api/auth/verify-email?token=zzz', undefined, { accept: 'text/html' });
    expect(badHtml.status).toBe(400);
    expect(badHtml.body).toContain('invalid');
    expect((await f.call('GET', '/api/auth/verify-email')).json).toEqual({ error: 'INVALID_TOKEN' });
  });
  ...
  it('logs in only verified users; brakes repeated failures except for signed synthetic runs', async () => {
    ...
    const login = () => f.call('POST', '/api/auth/login', { email: good.email, password: good.password });
    ...
    for (let i = 0; i < 5; i++) await f.call('POST', '/api/auth/login', { email: good.email, password: 'wrong' });
    expect((await login()).status).toBe(429);
    const run = { 'x-synthetic-run': signRunHeader('r1', SECRET, NOW) };
    expect((await f.call('POST', '/api/auth/login', { email: good.email, password: good.password }, run)).status).toBe(200);
  });
});
