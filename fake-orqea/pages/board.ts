...
export function boardPages(app: FastifyInstance, deps: Deps): void {
  const { store } = deps;
  const notFound = (t: (k: 'notFound') => string): Rendered => ({
    title: t('notFound'),
    body: `<h1>${esc(t('notFound'))}</h1>`,
    status: 404,
  });
  const h = { store, notFound };
  boardPages1(app, deps, h);
  boardPages2(app, deps, h);
  boardPages3(app, deps, h);
}

function boardPages1(
  app: FastifyInstance,
  deps: Deps,
  h: { store: Store; notFound: (t: (k: 'notFound') => string) => Rendered },
): void {
  const { store, notFound } = h;
  appPage(app, deps, '/boards', (ctx) => {
    const { t } = ctx.view;
    const boards = store
      .boardsOf(ctx.user.id)
      .map((b) => `<li><a href="/boards/${b.id}">${esc(b.name)}</a></li>`)
      .join('');
    const create = apiForm(
      'POST /api/boards',
      `${field(t('boardName'), 'name', { required: true })}${button(ctx.view, 'createBoard')}`,
      { redirect: '/boards/{id}' },
    );
    return {
      title: t('yourBoards'),
      body: `<h1>${esc(t('yourBoards'))}</h1><ul>${boards}</ul>${create}`,
    };
  });
  ...
  appPage(app, deps, '/boards/:boardId', (ctx, req) => {
    ...
    const lists = store.listsOf(board.id).map((l, i) => {
      const cards = store
        ...
        .map(
          (c) =>
            `<li><input type="checkbox" name="cardIds" value="${c.id}" data-collect data-bulk hidden aria-label="${esc(`${t('select')} ${c.title}`)}" form="bulk"> <a href="/cards/${c.id}" draggable="true" data-card-id="${c.id}">${esc(c.title)}</a></li>`,
        )
        .join('');
      const add =
        i === 0
          ? apiForm(
              `POST /api/lists/${l.id}/cards`,
              `${field(t('cardTitle'), 'title', { required: true })}${button(ctx.view, 'addCard', { icon: true })}`,
              { done: 'card' },
            )
          : '';
    ...
    const addList = apiForm(
      ...
      `${field(t('listName'), 'name', { required: true })}${button(ctx.view, 'addList')}`,
      { done: 'list' },
    );
...
${apiForm('POST /api/cards/bulk', `<input type="hidden" name="action" value="priority"><input type="hidden" name="value" value="high">${button(ctx.view, 'bulkHigh', { attrs: 'data-bulk hidden' })}`, { done: 'bulk' }).replace('<form ', '<form id="bulk" ')}`;
    ...
    return {
      title: board.name,
      body: `<h1>${esc(board.name)}</h1>${nav}${bulk}<div>${lists.join('')}</div>${addList}`,
    };
  });
}

function boardPages2(
  app: FastifyInstance,
  deps: Deps,
  h: { store: Store; notFound: (t: (k: 'notFound') => string) => Rendered },
): void {
...
${select(
  ...
  card.priority,
)}${button(ctx.view, 'save')}`,
      { done: 'saved' },
    );
    const items = card.checklist
      .map((c) => `<li><label><input type="checkbox"> ${esc(c.text)}</label></li>`)
      .join('');
    const addItem = apiForm(
      `POST /api/cards/${card.id}/checklist`,
      `${field(t('newItem'), 'text', { required: true })}${button(ctx.view, 'addItem')}`,
      { done: 'item' },
    );
    const comments = card.comments.map((c) => `<li>${esc(c.text)}</li>`).join('');
    const addComment = apiForm(
      `POST /api/cards/${card.id}/comments`,
      `${field(t('comment'), 'text', { textarea: true, required: true })}${button(ctx.view, 'postComment')}`,
      { done: 'comment' },
    );
...
${select(t('then'), 'action', [['archive', t('thenArchive')]], 'archive')}${button(ctx.view, 'createRule')}`;
    return {
      title: t('automations'),
      body: `<h1>${esc(t('automations'))}</h1><ul>${rules}</ul>${apiForm(`POST /api/boards/${board.id}/automations`, inner, { done: 'rule' })}`,
    };
  });
}

function boardPages3(
  app: FastifyInstance,
  deps: Deps,
  h: { store: Store; notFound: (t: (k: 'notFound') => string) => Rendered },
): void {
  ...
  appPage(app, deps, '/boards/:boardId/members', (ctx, req) => {
    ...
    const invite = apiForm(
      ...
      `${field(t('inviteEmail'), 'email', { required: true })}${button(ctx.view, 'sendInvite')}`,
      { done: 'invite' },
    );
    const guest = apiForm(
      `POST /api/boards/${board.id}/guest-links`,
      button(ctx.view, 'guestLink'),
      { done: 'guest' },
    );
    return {
      title: t('members'),
      body: `<h1>${esc(t('members'))}</h1><ul>${members}</ul>${invite}${guest}`,
    };
  });
}
