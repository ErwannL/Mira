...
export function publicPages(app: FastifyInstance, deps: Deps): void {
  ...
  publicPage(app, deps, '/signup', (ctx) => {
    ...
    const extras = EXTRA_FIELDS.slice(0, s.extraSignupFields)
      ...
      .join('');
    const captcha = s.captcha && !ctx.runId ? `<label><input type="checkbox" name="captcha"> ${esc(t('notRobot'))}</label>` : '';
...
${button(ctx.view, 'logIn')}`;
    return { title: t('logIn'), body: `<h1>${esc(t('logIn'))}</h1>${apiForm('POST /api/auth/login', inner, { redirect: '/boards' })}` };
  });
  ...
  publicPage(app, deps, '/f/:formId', (ctx, req) => {
    ...
    const questions = form.questions.map((q, i) => `<p>${esc(q)}</p>${field(i === 0 ? t('yourAnswer') : `${t('yourAnswer')} ${i + 1}`, 'answers', { extra: 'data-array' })}`).join('');
    return {
      title: form.title,
  ...
  publicPage(app, deps, '/account-deleted', ({ view: { t } }) => ({ title: t('accountDeleted'), body: `<h1>${esc(t('accountDeleted'))}</h1>` }));
}
