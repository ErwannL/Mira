...
export const REPORT_CSP = "default-src 'none'; style-src 'unsafe-inline'; img-src data:; frame-ancestors 'none'; sandbox";

export function securityHooks(app: FastifyInstance, o: { loopbackOnly: boolean; consoleOrigins: string[] }): void {
  ...
  app.addHook('onSend', async (req, reply, payload) => {
    ...
    if (req.url.startsWith('/api') || req.url.startsWith('/auth')) reply.header('cache-control', 'no-store');
    return payload;
  });
}
