...
export async function makeApp(overrides: Partial<AppConfig> = {}, nowS = () => Math.floor(Date.now() / 1000)) {
  ...
  const req = async (method: string, url: string, o: { body?: unknown; cookie?: string; headers?: Record<string, string> } = {}) => {
    const res = await app.inject({ method: method as 'GET', url, payload: o.body as object, headers: { ...H, ...(o.cookie ? { cookie: o.cookie } : {}), ...(o.headers ?? {}) } });
    ...
    return { status: res.statusCode, json, body: res.body, headers: res.headers, cookies: res.cookies };
  };
  /** Signs in through SSO like the console would; returns the session cookie header. */
  const login = async (operator = 'Ops Alice') => {
    const r = await req('POST', '/auth/sso', { body: { token: mintSsoToken(SSO, 'figura', operator, nowS()) } });
    const c = r.cookies.find((x) => x.name === 'figura_session')!;
    return `figura_session=${c.value}`;
  };
  const api = async (method: string, url: string, cookie: string, body?: unknown) => req(method, url, { cookie, body, headers: { 'x-figura': '1' } });
  return { app, db, cfg, req, login, api, close: async () => { await app.close(); await db.end(); } };
}
