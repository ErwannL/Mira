...
describe('SSO from the admin console', () => {
  it('a valid token opens a short httpOnly SameSite=Strict session; the token is single use', async () => {
    ...
    expect((await h!.req('POST', '/auth/sso', { body: { token } })).json).toEqual({ error: 'REUSED' });
  });
  ...
  it('refuses expired, wrong aud/iss, bad signature, long-lived and operator-less tokens', async () => {
    ...
    const cases: [string, string][] = [
      ...
      [signJwt({ iss: 'orqea-admin-console', aud: 'figura', exp: t + 3600, operator: 'x' }, SSO), 'BAD_LIFETIME'],
      [signJwt({ iss: 'orqea-admin-console', aud: ['a', 'figura'], iat: t - 100, exp: t + 30, operator: 'x' }, SSO), 'BAD_LIFETIME'],
      [signJwt({ iss: 'orqea-admin-console', aud: 'figura', exp: t + 30 }, SSO), 'NO_OPERATOR'],
      [signJwt({ iss: 'orqea-admin-console', aud: 'figura', exp: 'soon', operator: 'x' }, SSO), 'EXPIRED'],
      ['garbage', 'MALFORMED'],
    ];
    ...
    expect((await h!.req('POST', '/auth/sso', { body: { token: 'x'.repeat(5000) } })).status).toBe(400);
  });

  it('accepts an audience array and trims the operator name to 120 chars', () => {
    const t = now();
    const r = verifySsoToken(signJwt({ iss: 'orqea-admin-console', aud: ['figura'], iat: t, exp: t + 60, operator: 'o'.repeat(300) }, SSO), { secret: SSO, appId: 'figura', nowS: t });
  ...
  it('cross-site console: SameSite=None; Secure', async () => {
    ...
    const r = await h!.req('POST', '/auth/sso', { body: { token: mintSsoToken(SSO, 'figura', 'x', now()) } });
  ...
  it('without a session every API route answers 401; logout ends the session', async () => {
    ...
    expect((await h!.req('GET', '/api/runs', { cookie: 'figura_session=forged' })).status).toBe(401);
...
describe('loopback lock and headers', () => {
  it('a non-local Host gets 404, not 403, even on open routes', async () => {
    ...
    for (const url of ['/health', '/auth/sso', '/api/runs', '/']) {
      const r = await h!.app.inject({ method: 'GET', url, headers: { host: 'figura.example.com' } });
      expect(r.statusCode, url).toBe(404);
    }
    expect((await h!.app.inject({ method: 'GET', url: '/health', headers: { host: '127.0.0.1:4000' } })).statusCode).toBe(200);
  });

  it('can be unlocked behind an authenticating reverse proxy', async () => {
    h = await makeApp({ loopbackOnly: false });
    expect((await h!.app.inject({ method: 'GET', url: '/health', headers: { host: 'figura.internal' } })).statusCode).toBe(200);
  });
  ...
  it('sends frame-ancestors (never X-Frame-Options: DENY), strict CSP, no CORS', async () => {
    ...
    expect(r.headers['content-security-policy']).toContain('frame-ancestors http://localhost:4100 https://admin.orqea.test');
