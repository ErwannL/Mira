...
export class DbMemoryStore implements MemoryStore {
  ...
  async load(
    personaId: string,
  ): Promise<{ memory: Memory; credentials: Credentials | null } | null> {
    ...
    const credentials = row.credentials_enc
      ? (JSON.parse(decrypt(row.credentials_enc, this.key)) as Credentials)
      : null;
...
export async function memoriesOf(db: Db, runId: string): Promise<Memory[]> {
  const { rows } = await db.query<{ memory: Memory }>(
    'select memory from persona_memory where run_id = $1 order by persona_id',
    [runId],
  );
