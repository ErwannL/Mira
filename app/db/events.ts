...
export class EventWriter {
  ...
  private seq = 0;
  constructor(private readonly db: Db, private readonly runId: string, private readonly batchSize = 50) {}
  ...
  async flush(): Promise<void> {
    ...
    const rows = batch.map((e, i) => {
      values.push(this.runId, this.seq + i, e.personaId, e.kind, e.useCaseId, e.simTime, e.wallTime, e);
      const b = i * 8;
      return `($${b + 1}, $${b + 2}, $${b + 3}, $${b + 4}, $${b + 5}, $${b + 6}, $${b + 7}, $${b + 8})`;
    });
    this.seq += batch.length;
    await this.db.query(`insert into run_events (run_id, seq, persona_id, kind, use_case_id, sim_time, wall_time, payload) values ${rows.join(', ')}`, values);
  }
}

export async function eventsOf(db: Db, runId: string, personaId: string | null = null): Promise<StoredEvent[]> {
