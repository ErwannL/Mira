...
import { FINAL_STATUSES, TRANSITIONS, type RunConfig, type RunStatus } from '../../shared/run-config.js';
...
export async function createRun(db: Db, id: string, config: RunConfig, seed: number, actor: string): Promise<RunRow> {
  ...
  await db.query(`insert into run_transitions (run_id, from_status, to_status, actor, note) values ($1, null, 'draft', $2, 'created')`, [id, actor]);
...
export async function listRuns(db: Db, limit = 50): Promise<RunRow[]> {
  const { rows } = await db.query<RunRow>('select * from runs order by created_at desc limit $1', [limit]);
...
export async function transition(db: Db, id: string, to: RunStatus, actor: string, note: string | null = null, extra: Partial<Pick<RunRow, 'refusal_code' | 'refusal_message' | 'error' | 'summary' | 'catalogue_version' | 'weights_version' | 'target_version'>> = {}): Promise<RunRow> {
  ...
  await db.query('insert into run_transitions (run_id, from_status, to_status, actor, note) values ($1, $2, $3, $4, $5)', [id, row.previous, to, actor, note]);
...
export async function claimNext(db: Db, worker: string): Promise<RunRow | null> {
  ...
  try {
    ...
    const run = rows[0];
    if (run) {
      await client.query(`insert into run_transitions (run_id, from_status, to_status, actor, note) values ($1, 'queued', 'preparing', $2, 'claimed')`, [run.id, worker]);
    }
...
export async function requestCancel(db: Db, id: string, actor: string): Promise<RunRow | null> {
  ...
  if (run.status === 'draft' || run.status === 'queued') return transition(db, id, 'cancelled', actor, 'cancelled before start');
  return getRun(db, id);
}

export async function isCancelRequested(db: Db, id: string): Promise<boolean> {
  const { rows } = await db.query<{ cancel_requested: boolean }>('select cancel_requested from runs where id = $1', [id]);
...
export async function transitionsOf(db: Db, id: string): Promise<{ from_status: string | null; to_status: string; actor: string; note: string | null; at: Date }[]> {
  const { rows } = await db.query('select from_status, to_status, actor, note, at from run_transitions where run_id = $1 order by id', [id]);
  return rows;
}

export async function deleteRun(db: Db, id: string): Promise<boolean> {
  const { rowCount } = await db.query(`delete from runs where id = $1 and status = any($2::text[])`, [id, FINAL_STATUSES]);
  return rowCount === 1;
}
