...
export type AnyReport = FunnelReport | ComparisonReport | LoadReport | PricingReport | CalibrationReport;
...
const list = (items: string[], empty: string): string => (items.length ? `<ul>${items.map((i) => `<li>${esc(i)}</li>`).join('')}</ul>` : `<p>${esc(empty)}</p>`);

function metaBlock(m: ReportMeta, t: (k: I18nKey, p?: Record<string, string | number>) => string): string {
  const rows: [I18nKey, string | number][] = [['report.seed', m.seed], ['report.catalogue', m.catalogueVersion], ['report.weights', m.weightsVersion], ['report.target', `${m.targetVersion} (${m.targetUrl})`], ['report.duration', `${Math.round(m.durationMs / 1000)} s`]];
...
<h3>${esc(t('report.personas'))}</h3>${table(['id', '%'], m.personas.map((p) => [`${p.displayName} (${p.id})`, Math.round(p.weight * 1000) / 10]))}</section>`;
}
...
<section><h2>${esc(t('report.funnel.useCase'))}</h2>${table(
    [t('report.funnel.useCase'), t('report.funnel.attempted'), t('report.funnel.succeeded'), t('report.funnel.abandoned'), t('report.funnel.medianFriction'), t('report.funnel.reasons'), t('report.funnel.abandonShot')],
    r.useCases.map((u) => [u.title[locale], u.attempted, u.succeeded, u.abandoned, u.medianFriction, u.topAbandonReasons.map((x) => `${x.key} ×${x.count}`).join(', ') || '—', shot(u.abandonScreenshot)]),
  )}</section>
<section><h2>${esc(t('report.funnel.personaTable'))}</h2>${table(
    ['id', t('report.funnel.stage'), t('report.funnel.sessions'), t('inspector.frustration'), t('inspector.rule'), t('report.funnel.money')],
    r.personas.map((p) => [p.id, p.stage, p.sessions, p.frustration, p.lastRule ?? '—', p.money.join('; ') || '—']),
  )}</section>
...
function compare(r: ComparisonReport, t: (k: I18nKey) => string): string {
  const who = (xs: ComparisonReport['hurt']) => list(xs.map((x) => `${x.personaId} · ${x.useCaseId}: ${x.why}`), t('report.none'));
...
<section><h2>${esc(t('report.compare.funnel'))}</h2>${table([t('report.funnel.step'), 'A', 'B', 'Δ'], r.funnelDelta.map((d) => [d.id, d.a, d.b, d.delta]))}</section>
<section><h2>${esc(t('report.compare.friction'))}</h2>${table([t('report.funnel.useCase'), 'A', 'B', 'Δ'], r.frictionDelta.map((d) => [d.id, d.a, d.b, d.delta]))}</section>
<section><h2>${esc(t('report.compare.hurt'))}</h2>${who(r.hurt)}<h2>${esc(t('report.compare.helped'))}</h2>${who(r.helped)}</section>`;
}

function load(r: LoadReport, t: (k: I18nKey, p?: Record<string, string | number>) => string): string {
  const scenarios = r.scenarios
    .map((s) => `<section><h2>${esc(t('report.load.users', { n: s.users }))}</h2><p>${esc(t('report.load.objects'))}: ${s.objectsPerWeek} · ${esc(t('report.load.storage'))}: ${Math.round(s.storageGrowthBytesPerWeek / 1024)} KiB</p>
${table([t('report.load.endpoint'), t('report.load.perWeek'), t('report.load.peak')], s.endpoints.map((e) => [e.endpoint, e.requestsPerWeek, e.peakRequestsPerSecond]))}</section>`)
    .join('');
  return `<h1>${esc(t('report.load.title'))}</h1>${metaBlock(r.meta, t)}${scenarios}
<section><h2>${esc(t('report.load.latency'))}</h2>${table([t('report.load.endpoint'), 'n', 'p50', 'p95', 'p99'], r.measuredLatency.map((l) => [l.endpoint, l.count, l.p50, l.p95, l.p99]))}</section>`;
}

function pricing(r: PricingReport, t: (k: I18nKey) => string): string {
  const s = r.scenarios.map((sc) => `<section><h2>${esc(t('report.pricing.scenario'))}: ${esc(sc.name)}</h2><p>${esc(t('report.pricing.conversion'))}: ${sc.weightedConversion} · ${esc(t('report.pricing.revenue'))}: ${sc.revenuePer1000Users} €</p>
${table(['id', t('inspector.decision'), 'plan', '€'], sc.personas.map((p) => [p.id, p.decision, p.planKey ?? '—', p.monthlyCost]))}</section>`).join('');
  return `<h1>${esc(t('report.pricing.title'))}</h1>${metaBlock(r.meta, t)}${s}<section><h2>${esc(t('report.pricing.triggers'))}</h2>${list(r.paywallTriggers.map((x) => `${x.featureKey} ×${x.count}`), t('report.none'))}</section>`;
}

function calibration(r: CalibrationReport, t: (k: I18nKey) => string): string {
  return `<h1>${esc(t('report.calibration.title'))}</h1><p>${esc(r.note)}</p>${table([t('report.funnel.step'), t('report.calibration.simulated'), t('report.calibration.real'), t('report.calibration.delta')], r.rows.map((x) => [x.step, x.simulated, x.real, x.delta]))}
<h2>${esc(t('report.calibration.suggestions'))}</h2>${list(r.suggestions.map((s) => `${s.direction} ${s.personaId}: ${s.because}`), t('report.none'))}`;
}
...
export function renderReportHtml(report: AnyReport, locale: Locale, img: ImageSource): string {
  ...
  const body =
    report.type === 'funnel' ? funnel(report, locale, t, img)
    : report.type === 'comparison' ? compare(report, t)
    : report.type === 'load' ? load(report, t)
    : report.type === 'pricing' ? pricing(report, t)
