...
export interface PricingReport {
  ...
  scenarios: {
    ...
    personas: {
      id: string;
      decision: MoneyDecision | 'no-encounter';
      planKey: string | null;
      monthlyCost: number;
    }[];
...
export function buildPricing(
  ...
  memories: Memory[],
  plans: Plan[],
  alternatives: { name: string; prices: Record<string, number> }[],
): PricingReport {
  const memo = new Map(memories.map((m) => [m.personaId, m]));
  const scenarios = [{ name: 'target', prices: {} as Record<string, number> }, ...alternatives].map(
    (s) => {
      const priced = reprice(plans, s.prices);
      const rows = personas.map((p) => {
        const outcomes = (memo.get(p.id)?.money ?? []).map((m) =>
          decideMoney(m.encounter, priced, p),
        );
        const convert = outcomes.find((o) => o.decision === 'convert');
        const decision = convert
          ? 'convert'
          : outcomes.some((o) => o.decision === 'churn')
            ? 'churn'
            : outcomes.length
              ? 'defer'
              : 'no-encounter';
        return {
          id: p.id,
          decision: decision as MoneyDecision | 'no-encounter',
          planKey: convert?.planKey ?? null,
          monthlyCost: convert?.monthlyCost ?? 0,
      ...
      return {
        ...
        revenuePer1000Users: round4(
          converted.reduce((a, r) => a + r.weight * 1000 * r.monthlyCost, 0),
        ),
      };
    },
  );
  const triggers = countBy(
    memories.flatMap((m) => m.money.filter((x) => x.encounter.featureKey !== null)),
    (x) => x.encounter.featureKey as string,
  );
  return {
    type: 'pricing',
    meta,
    plans,
    scenarios,
    paywallTriggers: Object.entries(triggers)
