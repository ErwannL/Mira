...
describe('stats', () => {
  it('quantiles, medians, rounding, counting, top-n with deterministic ties', () => {
    ...
    expect(top({ b: 2, a: 2, c: 5 }, 2)).toEqual([
      { key: 'c', count: 5 },
      { key: 'a', count: 2 },
    ]);
  });
...
describe('funnel report', () => {
  ...
  it('shows who abandoned where, with the screenshot and reasons, and the coverage matrix', async () => {
    ...
    expect(signup.personas['retired-volunteer']).toMatchObject({
      attempted: true,
      succeeded: false,
      abandoned: true,
    });
  ...
  it('copes with personas that never lived (no memory, no events)', () => {
    ...
    expect(r.personas[0]).toMatchObject({
      stage: 'new',
      sessions: 0,
      frustration: 0,
      succeeded: [],
  ...
  it('abandon reasons fall back to the rule when friction had no reason', () => {
    const e = {
      ...
      wallMs: 1,
      facts: null,
      friction: { score: 0, reasons: [] },
      frustration: 1,
      action: 'abandon' as const,
      rule: 'hard block: control-without-accessible-name',
      mistakes: [],
      screenshot: null,
      apiCalls: [],
      money: null,
    ...
    const r = buildFunnel(
      ...
      catalogue,
      samplePersonas,
      [e, { ...e, friction: null, rule: 'paywall churn: x' }],
      [],
      [],
    );
    expect(r.useCases.find((u) => u.id === 'signup')!.topAbandonReasons.map((x) => x.key)).toEqual([
      'hard block',
      'paywall churn',
    ]);
  });
...
describe('comparison (A/B)', () => {
  it('finds who got hurt and helped, friction and funnel deltas', async () => {
    const a = buildFunnel(
      ...
      catalogue,
      samplePersonas,
      ...(Object.values(await sampleRun(3)) as [never, never]),
      endpoints,
    );
    const b = buildFunnel(
      { ...sampleMeta, seed: 1 },
      catalogue,
      samplePersonas,
      ...(Object.values(await sampleRun(6)) as [never, never]),
      endpoints,
    );
    const c = buildComparison(a, b);
    expect(c.sameSeed).toBe(false);
    expect(c.hurt).toContainEqual({
      personaId: 'retired-volunteer',
      useCaseId: 'signup',
      why: 'abandoned in B',
    });
    ...
    expect(back.helped).toContainEqual({
      personaId: 'retired-volunteer',
      useCaseId: 'signup',
      why: 'succeeds only in B',
    });
  ...
  it('flags friction increases above the threshold and missing rows', async () => {
    ...
    landing.personas['student']!.medianFriction =
      (landing.personas['student']!.medianFriction ?? 0) + 0.5;
    ...
    expect(c.frictionDelta.find((d) => d.id === 'account-delete')).toEqual({
      id: 'account-delete',
      a: null,
      b: null,
      delta: null,
...
describe('load forecast', () => {
  it('extrapolates per user scenario and measures latency percentiles', async () => {
    ...
    expect(s100!.storageGrowthBytesPerWeek).toBe(
      Math.round(s100!.objectsPerWeek * BYTES_PER_OBJECT),
    );
  ...
  it('maps volume clones back to their persona and handles tiny/empty runs', () => {
    ...
    const clone = {
      ...
      mistakes: [],
      screenshot: null,
      apiCalls: [{ method: 'POST', path: '/api/auth/register', status: 201, ms: 5 }],
      money: null,
      error: null,
    };
    const r = buildLoad(
      { ...sampleMeta, simulatedDays: 0.1 },
      samplePersonas,
      [clone, { ...clone, personaId: 'student-c2' }],
      timeConfig,
      [7],
    );
    expect(r.scenarios[0]!.objectsPerWeek).toBeGreaterThan(0);
    expect(
      buildLoad(sampleMeta, samplePersonas, [], timeConfig, [10]).scenarios[0]!.endpoints,
    ).toEqual([]);
  });
});
...
describe('pricing', () => {
  it('replays money decisions under target prices and alternatives', async () => {
    ...
    const r = buildPricing(sampleMeta, samplePersonas, memories, testPlans, [
      { name: 'cheap', prices: { pro: 1 } },
      { name: 'dear', prices: { pro: 500 } },
    ]);
  ...
  it('records churn and defer outcomes', async () => {
    const { memories } = await sampleRun(3);
    const expensive = buildPricing(
      sampleMeta,
      samplePersonas.map((p) => ({ ...p, valueThreshold: 1 })),
      memories,
      testPlans,
      [{ name: 'dear', prices: { pro: 500 } }],
    );
...
describe('calibration', () => {
  ...
  it('compares and suggests weight changes, never applying them', async () => {
    ...
    const tooOptimistic = buildCalibration(f, {
      funnel: { landing: 1, signup: 0.2, login: 0.95, 'create-board': 0.1 },
    });
    expect(tooOptimistic.rows.find((r) => r.step === 'verify-email')).toMatchObject({
      real: null,
      delta: null,
    });
    expect(tooOptimistic.suggestions).toContainEqual(
      expect.objectContaining({ personaId: 'retired-volunteer', direction: 'increase' }),
    );
    expect(tooOptimistic.suggestions).toContainEqual(
      expect.objectContaining({ personaId: 'project-manager', direction: 'decrease' }),
    );
...
describe('HTML export', () => {
  it('renders every report type self-contained, in English and French, escaping content', async () => {
    ...
    const html = renderReportHtml(
      { ...funnel, personas: [{ ...funnel.personas[0]!, lastRule: '<b>x</b>' }] },
      'en',
      img,
    );
    ...
    expect(
      renderReportHtml(buildLoad(sampleMeta, samplePersonas, events, timeConfig, [100]), 'en', img),
    ).toContain('100 users');
    expect(
      renderReportHtml(buildLoad(sampleMeta, samplePersonas, [], timeConfig, [5]), 'en', img),
    ).not.toContain('<table><thead><tr><th>Endpoint');
    expect(
      renderReportHtml(
        buildPricing(sampleMeta, samplePersonas, memories, testPlans, []),
        'en',
        img,
      ),
    ).toContain('Weighted conversion');
    expect(
      renderReportHtml(buildCalibration(funnel, { funnel: { signup: 0.9 } }), 'en', img),
    ).toContain('Suggested persona-weight changes');
  });
});
