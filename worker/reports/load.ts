...
export function buildLoad(
  ...
  for (const e of events) {
    ...
    const map =
      perPersona.get(base) ??
      (perPersona.set(base, new Map()).get(base) as Map<
        string,
        { calls: number; created: number }
      >);
  ...
  const scenarios = userScenarios.map((users) => {
    ...
    const endpoints = [...totals.entries()]
      ...
      .map(([endpoint, t]) => ({
        endpoint,
        requestsPerWeek: round4(t.perWeek),
        peakRequestsPerSecond: round4(t.peak),
      }));
    const objects = round4([...totals.values()].reduce((s, t) => s + t.created, 0));
    return {
      users,
      endpoints,
      objectsPerWeek: objects,
      storageGrowthBytesPerWeek: Math.round(objects * BYTES_PER_OBJECT),
    };
  });
  ...
  return {
    type: 'load',
    meta,
    assumptions: { bytesPerObject: BYTES_PER_OBJECT, peakMultiplier: peak },
    scenarios,
    measuredLatency,
  };
}
