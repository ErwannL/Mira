...
`loop.ts` claims runs (SKIP LOCKED) and recovers runs abandoned by a dead worker. `runner.ts`: preparing (target guard, drift check) → running (simulation) → reporting → cleaning (always) → done/failed/refused/cancelled. `modes.ts` builds journey (browser) or volume (API clones, rate-limited) drivers. `build-reports.ts` stores reports. `start.ts`/`main.ts` read the environment.
