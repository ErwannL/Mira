...
function driver(overrides: Partial<ConstructorParameters<typeof ApiDriver>[0]> = {}) {
  return new ApiDriver({ baseUrl: fake.baseUrl, runHeader: () => fake.runHeader(), fetchImpl: fetch, now: Date.now, limiter: null, ...overrides });
}
const ctx = (vars: Record<string, string>, mistakes: AttemptContext['mistakes'] = []): AttemptContext => ({ persona: p, vars, mistakes, label: 't' });

describe('ApiDriver', () => {
  it('lives signup → verify → login → board → card through the API', async () => {
    const d = driver();
    const vars: Record<string, string> = { email: 'synth+r1-student@synthetic.invalid', password: 'Str0ngPassword1', boardName: 'Uni', cardTitle: 'Essay' };
    const signup = await d.attempt(uc('signup'), ctx(vars));
    expect(signup.ok).toBe(true);
    expect(signup.apiCalls[0]).toMatchObject({ method: 'POST', path: '/api/auth/register', status: 201 });
    ...
    expect(paywalled).toMatchObject({ ok: false, paywall: { code: 'FEATURE_LOCKED', featureKey: 'automation' } });
  ...
  it('counts unclear messages, captcha that would show, and plan limits without a feature', async () => {
    ...
    const d = new ApiDriver({ baseUrl: f2.baseUrl, runHeader: () => f2.runHeader(), fetchImpl: fetch, now: Date.now, limiter: null });
    const vars: Record<string, string> = { email: 'synth+r1-x@synthetic.invalid', password: 'Str0ngPassword1', boardName: 'A' };
  ...
  it('network failures, 5xx, non-JSON bodies and odd 402s', async () => {
    ...
    const down = driver({ fetchImpl: (async () => { throw new Error('ECONNREFUSED'); }) as typeof fetch });
    ...
    const plain = await driver({ fetchImpl: respond(200, '{"id":1}') }).attempt(uc('landing'), ctx(vars));
    expect(plain.ok).toBe(true);
    const noMessage = await driver({ fetchImpl: respond(400, '{}') }).attempt(uc('signup'), ctx(vars));
    expect(noMessage.facts.unclearErrors).toBe(1);
    const missing = await driver({ fetchImpl: respond(201, '{}') }).attempt(uc('create-board'), ctx(vars));
...
describe('RateLimiter', () => {
  it('lets a burst through then waits for tokens', async () => {
    let t = 0;
    const sleeps: number[] = [];
    const l = new RateLimiter(2, () => t, async (ms) => {
      sleeps.push(ms);
      t += ms;
    });
    await l.take();
