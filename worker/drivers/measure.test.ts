...
describe('measurePage', () => {
  it('counts visible fields, required ones and words; ignores hidden/script/style', () => {
    const m = measure(`<form><label>Email <input name="e" required></label><label>Pw <input type="password" aria-required="true"></label>
  ...
  it('finds controls without accessible names, by role', () => {
    const m = measure(`<button><svg aria-hidden="true"></svg></button><a href="/x"></a><input><input type="checkbox">
    ...
    expect(m.unnamedByRole).toEqual({ button: 2, link: 1, textbox: 2, checkbox: 1, radio: 1, searchbox: 1, combobox: 1, tab: 1 });
  ...
  it('detects captcha, cookie banner, terms checkbox', () => {
    ...
    const iframe = measure(`<iframe src="https://captcha.example/x"></iframe><div role="alertdialog" hidden>cookie</div>`);
  ...
  it('reads validation errors and judges their clarity', () => {
    const m = measure(`<p role="alert">Error.</p><p role="alert">Enter an email address like name@example.com.</p><p role="alert"> </p><p role="alert" hidden>x</p>`);
  ...
  it('flags text in the wrong language', () => {
    const english = '<p>Create your account and invite the team to your boards with the plan for this</p>';
    const french = '<p>Créez votre compte et invitez les membres de votre équipe pour une meilleure organisation des tâches</p>';
  ...
  it('detects horizontal overflow', () => {
    ...
    expect(measurePage(doc, win as unknown as globalThis.Window, 'en').horizontalOverflow).toBe(true);
    expect(measure('<p>x</p>').horizontalOverflow).toBe(false);
  });

  it('serialises into an injectable script', () => {
    expect(measureScript('fr')).toMatch(/^\(function measurePage[\s\S]*\)\(document, window, "fr"\)$/);
  });
});
