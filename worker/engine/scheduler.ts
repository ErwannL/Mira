...
export function activationProbability(
  ...
  return round(
    Math.min(1, base * (0.5 + persona.activityLevel) * hourMultiplier(hour, cfg) * inHours),
  );
}
...
export function isActive(
  ...
  return {
    active: roll < probability,
    probability,
    roll,
    localHour: localHour(at, persona.timezone),
  };
}

/** Simulated instants of each round. */
export function roundTimes(
  start: Date,
  ...
  return Array.from(
    { length: count },
    (_, i) => new Date(start.getTime() + i * minutesPerRound * 60_000),
  );
}
