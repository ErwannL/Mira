...
const rules = [
  [
    /\b(it|test|describe)\.(skip|only|todo)\b|\b(xit|fit|xdescribe|fdescribe)\(/,
    'skipped/focused test',
  ],
...
const problems = [];
for (const f of repoFiles().filter(
  (f) => /\.(ts|mjs|js)$/.test(f) && !f.startsWith('scripts/check-tests'),
)) {
